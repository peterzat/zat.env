...
- [x] **CLAUDE.md and README.md document the new mode.** CLAUDE.md gains an "External-only review pre-flight contract" bullet under "Prompt/infrastructure boundary" naming the dual-path contract (silent-exit-0 default vs. fail-loud `--check`) and the "no marker write in external mode" invariant. README.md's External Reviewers section gains a paragraph naming `/codereview external [<ref>|<range>]`, providing at least one example invocation (e.g. `/codereview external v1.3`), and explicitly noting the "no CODEREVIEW.md / no marker / no /codefix" semantics.
...
**What happened:** Shipped `/codereview external` end to end across three SKILL/script/docs commits (06cc69e, 096866f, 8cb06bc) plus a turn-close hardening commit (9533660) and the review-meta commit (cb72ae9). Code-side: Step 0 dispatch on the first arg token, External-Only Mode body with Steps E.1-E.5 (pre-flight `--check` gate, four-form range resolution incl. natural-language phrasings, marker-collision warning gated on default range only, diff compute with the same exclusion list as full review, parallel-provider invocation with `--range` plumbing, terminal-only output with no-mutation footer disclaimer). Script-side: `--check` and `--range` flags in `bin/review-external.sh` with full argument-arity validation (incl. the `--range ""` symmetric-validation fix landed this turn). Hardening side-effects from the same review cycle: marker dir migrated from `/tmp/.claude-codereview-<hash>` to `${XDG_CACHE_HOME:-${HOME}/.cache}/claude-codereview/` (mode 0700, per-user) closing a cross-user symlink-race vector, pre-push hook now fails closed (exit 2) on unexpected `codereview-marker` error so a missing-from-PATH script cannot silently bypass the gate, `bin/codereview-skip` sources its path via `codereview-marker skip-path` to single-source the marker layout. Closed the two carry-forward NOTEs (test executable bit, `hooks/README.md` marker-consumption wording). Refreshed README directory overview check counts (lint 230 → 378, hook 39 → 41, external 35 → 61) and added the two previously-missing test entries (test-spec-backlog-apply 64, test-codereview-marker 37). Added `/codereview external` to the README "Since v1.3 (ongoing)" Roadmap section. 583/583 tests green.

**Questions and directions for the next turn:**

1. **`/verify` skill.** Headline "Next up" item per the README Roadmap and a longstanding planned skill: execute the project's test suite as ground-truth signal, with a `FINDINGS.md` lifecycle and an optional `qpeek` human-eval bridge. Big enough to need its own spec turn; the codereview-gate work is now a clean handoff point for picking it up.

2. **Real-world exercise of `/codereview external`.** The mode is shipped but never used in anger; running `/codereview external v1.3` against this repo (or some downstream project against its own last release) would surface any prose-vs-behavior gaps the lint and behavioral tests miss. Lighter scope than `/verify`; suitable as a warm-up turn or a parallel exploration.

3. **Worktree-based A/B testing or loop orchestrator.** Both listed under README Roadmap "Next up" and "Future." Larger scope; arguably blocked on `/verify` shipping first so the loop has a ground-truth signal to converge on rather than relying on `/codereview` alone for verification.

<!-- SPEC_META: {"date":"2026-05-01","title":"/codereview external mode","criteria_total":8,"criteria_met":8} -->
